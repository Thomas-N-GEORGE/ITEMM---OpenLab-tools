...
const userStorage = "oltPoutresUserData";
...
$j(document).ready(function () {
  ...
  function calcGamma(data) {
    ...
    data.gamma =
      data.larg *
      Math.pow(data.epai, 3) *
      (1 / 3 -
        0.21 *
          (data.epai / data.larg) *
          (1 - Math.pow(data.epai, 4) / (12 * Math.pow(data.larg, 4))));
  }

  function calcI2(data) {
    // =((C4*C5)/12)*(C4*C4+C5*C5)
    data.i2 =
      ((data.larg * data.epai) / 12) *
      (data.larg * data.larg + data.epai * data.epai);
  }

  function calcK(data) {
    // =(C4*C5*C5*C5/16)/(16/3-3,36*(C5/C4)*(1-((C5*C5*C5*C5/(12*C4*C4*C4*C4)))))
    data.k =
      (data.larg * Math.pow(data.epai, 3)) /
      16 /
      (16 / 3 -
        3.36 *
          ((data.epai / data.larg) *
  ...
  function calcCphas(data) {
    ...
    data.cphas =
      Math.pow(
        (data.myou * 1000000000 * data.i1) /
          (data.mvol * (data.epai * data.larg)),
        0.25
      ) * Math.sqrt(2 * Math.PI * data.freqflex);
  }
  ...
  function calcFgui(data) {
    ...
    data.fgui =
      ((data.knl * data.knl) / (data.long * data.long * 2 * Math.PI)) *
      Math.sqrt(
        (data.fel * 1000000000 * data.i1) / (data.mvol * data.larg * data.epai)
      );
  }
  ...
  function calcPers(data) {
    ...
    data.pers =
      ((data.knl * data.knl) / (data.long * data.long * 2 * Math.PI)) *
      Math.sqrt(
        (data.pel * 1000000000 * data.i1) / (data.mvol * data.larg * data.epai)
      );
  }
  ...
  const poutres = new olt(numConstants, fields, userStorage, ["knl"]);
